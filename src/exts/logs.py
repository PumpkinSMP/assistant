...
class Logs(commands.Cog):
    ...
    def get_logs_channel(self, channel: str = None):
      channels = {
        "message": 1061902227565981738
      }
      return channels[channel]

    @commands.Cog.listener()
    async def on_message_delete(self, message: nextcord.Message):
      if message.channel.id in [1052178682426433600] and before.author.id in [960850854506815528]:
        return
      ...
      embed.add_field(name="Author", value=f"Mention: {message.author.mention}\nID: {message.author.id}", inline=False)
      embed.add_field(name="Channel", value=f"Mention: {message.channel.mention}\nID: {message.channel.id}", inline=False)
    ...
    async def on_message_edit(self, before: nextcord.Message, after: nextcord.Message):
      ...
      embed.add_field(name="Author", value=f"Mention: {before.author.mention}\nID: {before.author.id}", inline=False)
      embed.add_field(name="Channel", value=f"Mention: {before.channel.mention}\nID: {before.channel.id}", inline=False)
